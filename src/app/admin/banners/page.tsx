...
async function createBanner(formData: FormData) {
  ...
  await prisma.banner.create({
    data: { title, text, imageUrl, link: link || null }
  });
      ...
      <form action={createBanner} className="rounded-xl border border-black/5 bg-white p-6 shadow-soft space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-2 text-sm">
            ...
            <input name="title" placeholder="Summer Sale" required className="w-full rounded-lg border border-black/10 px-3 py-2" />
          </label>
          <label className="space-y-2 text-sm">
        ...
        <label className="space-y-2 text-sm">
          ...
          <textarea name="text" placeholder="Get up to 50% off" rows={2} className="w-full rounded-lg border border-black/10 px-3 py-2" />
        </label>
        <label className="space-y-2 text-sm">

...
async function getMetrics() {
  const [todayRevenue, weekRevenue, unfulfilled, lowStock] = await Promise.all([
    prisma.order.aggregate({
      _sum: { totalCents: true },
      where: { createdAt: { gte: new Date(new Date().setHours(0, 0, 0, 0)) }, status: 'PAID' }
    }),
    prisma.order.aggregate({
      _sum: { totalCents: true },
    ...
    prisma.variant.count({ where: { inventoryQty: { lt: 5 } } })
  ]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard label="Low stock" value={metrics.lowStock.toString()} />
      </div>
            ...
            <tbody>
              {recent.map((order) => (
                <tr key={order.id} className="border-t border-black/5">
                  ...
                  <td className="py-2 text-black/70">{order.email}</td>
                  <td className="py-2">{formatCurrency(order.totalCents)}</td>
                  <td className="py-2 text-black/70">{order.status}</td>
                </tr>
              ))}
            </tbody>
          </table>

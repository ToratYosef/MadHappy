...
import { markPrintifyPublishSucceeded, parsePrintifyError } from '@/lib/printify';
import type { PrintifyPublishSucceededRequest } from '@/types/printify';
...
export async function POST(req: Request, { params }: Params) {
  try {
    const body = (await req.json()) as PrintifyPublishSucceededRequest & { shopId?: string };
    const { shopId: shopIdOverride, ...payload } = body;
    const shopId = shopIdOverride || getShopId(req);

    const data = await markPrintifyPublishSucceeded({

...
import { markPrintifyPublishFailed, parsePrintifyError } from '@/lib/printify';
import type { PrintifyPublishFailedRequest } from '@/types/printify';
...
export async function POST(req: Request, { params }: Params) {
  try {
    const body = (await req.json()) as PrintifyPublishFailedRequest & { shopId?: string };
    const { shopId: shopIdOverride, ...payload } = body;
    const shopId = shopIdOverride || getShopId(req);

    const data = await markPrintifyPublishFailed({
